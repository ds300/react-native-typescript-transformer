...
module.exports.createTransformer = customOptions => {
  customOptions = customOptions || {}
  tsConfig = (() => {
    const customTsConfigPath = customOptions.tsConfigPath || TSCONFIG_PATH
    if (customTsConfigPath) {
      const resolvedTsconfigPath = path.resolve(
        process.cwd(),
        customTsConfigPath
      )
      ...
      console.warn(
        'tsconfig file specified by tsConfigPath option or TSCONFIG_PATH environment variable was not found'
      )
      console.warn(`tsConfigPath: ${customOptions.tsConfigPath}`)
  ...
  return async function(src, filename, options) {
